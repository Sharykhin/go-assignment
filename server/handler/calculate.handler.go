...
func CalculateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, switcher.ErrUnexpectedInput) {
			response.Fail(w, http.StatusBadRequest, errors.New("unexpected input for a, b, c"))
			return
		}
		if errors.Is(err, switcher.ErrUnexpectedMode) {
			response.Fail(w, http.StatusBadRequest, errors.New("mode must be either of base, custom1 or custom2"))
			return
		}
		response.Fail(w, http.StatusInternalServerError, err)
		return
	}
